...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function apiRequest(url, options = {}) {
        ...
        if (res.status === 401 && !isRefreshingToken) {
            ...
            try {
                const refreshRes = await fetch('https://quantum-6x3h.onrender.com/api/auth/refresh', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ token: refreshToken }) });
            ...
            } catch (error) { console.error("Token refresh failed:", error); showAuthView(); return null; } finally { isRefreshingToken = false; }
        }
        return res;
    }
    ...
    function appendMessage(msg, shouldScroll = true) {
        ...
        let replyHTML = '';
        if (msg.reply_to_content) { replyHTML = `<div class="message-reply"><strong>${msg.reply_to_username || '...'}</strong><p>${msg.reply_to_content}</p></div>`; }
    ...
    function hangUp() {
        if (peerConnection) { peerConnection.close(); peerConnection = null; callEndedSound.play().catch(e => {}); }
        stopCallTimer();
    ...
    function initWebSocket() {
        ...
        socket.onmessage = async (event) => {
            ...
            switch (data.type) {
                ...
                case 'new_message': await renderChats(); if (data.data.sender_id === selectedUserId || (data.data.receiver_id === selectedUserId && data.data.sender_id === currentUser.id)) { appendMessage(data.data); } if (data.data.sender_id !== currentUser.id && document.hidden && userSettings.soundNotifications) { notificationSound.play().catch(e => {}); } break;
    ...
    loginForm.addEventListener('submit', async (e) => { e.preventDefault(); try { const res = await fetch('https://quantum-6x3h.onrender.com/api/auth/login', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email: loginForm.querySelector('#login-email').value, password: loginForm.querySelector('#login-password').value }) }); const data = await res.json(); if (!res.ok) throw new Error(data.message); accessToken = data.accessToken; refreshToken = data.refreshToken; currentUser = data.user; localStorage.setItem('accessToken', accessToken); localStorage.setItem('refreshToken', refreshToken); localStorage.setItem('user', JSON.stringify(currentUser)); showToast('Вход выполнен успешно!', 'success'); showChatView(); } catch (error) { showToast(error.message || 'Ошибка входа', 'error'); } });
    registerForm.addEventListener('submit', async (e) => { e.preventDefault(); try { const res = await fetch('https://quantum-6x3h.onrender.com/api/auth/register', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ username: registerForm.querySelector('#username').value, email: registerForm.querySelector('#email').value, password: registerForm.querySelector('#password').value }) }); const data = await res.json(); if (!res.ok) throw new Error(data.message); showToast('Регистрация успешна! Теперь вы можете войти.', 'success'); switchAuthForm('login'); loginForm.querySelector('#login-email').value = registerForm.querySelector('#email').value; } catch (error) { showToast(error.message || 'Ошибка регистрации', 'error'); } });
    logoutButton.addEventListener('click', async () => { try { await apiRequest('/api/auth/logout', { method: 'POST', body: JSON.stringify({ token: refreshToken }) }); } catch(e) { console.error("Logout failed, clearing client-side anyway."); } finally { showAuthView(); } });
    ...
    messagesContainer.addEventListener('contextmenu', e => { e.preventDefault(); const messageBubble = e.target.closest('.message-content-wrapper'); if (!messageBubble) return; const messageWrapper = messageBubble.closest('.message-wrapper'); const isSent = messageWrapper.classList.contains('sent'); const isDeleted = messageWrapper.querySelector('.message-content.deleted'); if (isDeleted) return; messageContextMenu.querySelector('[data-action="edit"]').style.display = isSent ? 'block' : 'none'; messageContextMenu.querySelector('[data-action="delete"]').style.display = isSent ? 'block' : 'none'; contextMessage = messageWrapper; messageContextMenu.style.top = `${e.clientY}px`; messageContextMenu.style.left = `${e.clientX}px`; messageContextMenu.classList.remove('hidden'); });
    messageContextMenu.addEventListener('click', e => { if (!contextMessage) return; const action = e.target.dataset.action; const msgId = contextMessage.dataset.messageId; const contentEl = contextMessage.querySelector('.message-content'); if (action === 'delete') { socket.send(JSON.stringify({ type: 'delete_message', messageId: msgId, receiver_id: selectedUserId })); } else if (action === 'edit') { editingMessage = { id: msgId, content: contentEl.textContent }; editMessageInput.value = editingMessage.content; showModal(editMessageModal); } else if (action === 'reply') { replyingToMessage = { id: msgId, content: contentEl.textContent.substring(0, 50) + '...' }; replyTextPreview.textContent = replyingToMessage.content; replyPreviewBar.classList.remove('hidden'); messageInput.focus(); } messageContextMenu.classList.add('hidden'); });
    ...
    closeProfileModalBtn.addEventListener('click', () => { profileModalMobile.classList.remove('open'); });
    editProfileForm.addEventListener('submit', async (e) => { e.preventDefault(); try { const textUpdateRes = await apiRequest('/api/users/profile', { method: 'PUT', body: JSON.stringify({ username: usernameInput.value, bio: bioInput.value }) }); if (!textUpdateRes.ok) throw new Error('Ошибка обновления профиля'); if (avatarInput.files.length > 0) { const formData = new FormData(); formData.append('avatar', avatarInput.files[0]); const avatarUpdateRes = await apiUploadRequest('/api/users/profile/picture', { method: 'PUT', body: formData }); if (!avatarUpdateRes.ok) throw new Error('Ошибка загрузки аватара'); } closeModal(editProfileModal); showToast('Профиль успешно обновлен', 'success'); await renderChats(); await renderProfileSidebar(currentUser.id); } catch (error) { showToast(error.message, 'error'); } });
    ...
    document.body.addEventListener('change', async (e) => { if (e.target.matches('#photo-upload-input') && e.target.files.length > 0) { const formData = new FormData(); formData.append('photo', e.target.files[0]); const res = await apiUploadRequest('/api/photos', { method: 'POST', body: formData }); if (res.ok) { showToast('Фото загружено!', 'success'); await renderProfileSidebar(currentUser.id); } else { showToast('Ошибка загрузки фото.', 'error'); } e.target.value = ''; } if (e.target.matches('#image-upload-input') && e.target.files.length > 0) { const formData = new FormData(); formData.append('photo', e.target.files[0]); const res = await apiUploadRequest('/api/photos', { method: 'POST', body: formData }); if (!res.ok) return showToast('Ошибка загрузки', 'error'); const data = await res.json(); socket.send(JSON.stringify({ type: 'message', receiver_id: selectedUserId, content: data.filename, messageType: 'image' })); e.target.value = ''; } });
    ...
    function renderSearchResults(users) {
        ...
        if (users.length === 0) { searchResultsList.innerHTML = '<li>Пользователи не найдены.</li>'; return; }
        users.forEach(user => {
            const li = document.createElement('li');
            li.dataset.userId = user.id;
            ...
            li.addEventListener('click', () => { selectChat(user); searchInput.value = ''; searchResultsContainer.classList.add('hidden'); chatsListContainer.classList.remove('hidden'); });
    ...
    searchInput.addEventListener('input', () => {
        ...
        if (!query) { searchResultsContainer.classList.add('hidden'); chatsListContainer.classList.remove('hidden'); return; }
        searchTimeout = setTimeout(async () => {
            ...
            } catch (error) { console.error("Ошибка поиска:", error); showToast('Не удалось выполнить поиск', 'error'); }
        }, 300);
    });
